    ...
    <div id="beer-info">
      ...
      <div id="beer-text">
        ...
        <div id="ingredientsContainer">
          <div class="ingredient" v-for="(value, key) in ingredients" :key="key">
            <h3>{{key | capitalize}}</h3>
            <ul>
              <li v-for="(ingredient, index) in value" :key="index">{{ingredient}}</li>
            </ul>
          </div>
        </div>
      </div>
    ...
    ingredients(){
      ...
      for(let ingredientKey in this.beer.ingredients){
        if(typeof this.beer.ingredients[ingredientKey] === 'string'){
          ingredientData[ingredientKey] = [this.beer.ingredients[ingredientKey]]
        } else {
