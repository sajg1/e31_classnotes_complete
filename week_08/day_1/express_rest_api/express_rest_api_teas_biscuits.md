...
2. The form has a `v-model` attribute for each field, tracking the entered values

3. A URL and payload is constructed from the form details

4. A `fetch` request is sent. with the body sent as JSON (just as in Insomnia), the method set to `POST`, and the `Content-Type` set to "application/json". Each of these steps in necessary.

5. When the server responds, we use the `eventBus` to refresh the data and then we clear the form.
